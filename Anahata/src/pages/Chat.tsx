...
const Chat = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (stressMode === 'exam' || stressMode === 'work' || stressMode === 'internship') {
        response = "I know you're under a lot of pressure right now. Let's focus on grounding and reassurance. Take a deep breath. You're doing your best, and that's enough.";
      } else if (isNightMode) {
        response = "Late nights can be hard. I'm here, just listening. No need to figure everything out right now. How does it feel to just be in this moment?";
      } else {
        response = detectIndianContext(userInput);
      }
          ...
          <AnimatePresence>
            {messages.map((message, index) => (
              ...
              >
                <div
                  className={`max-w-[85%] ${
                    message.isAI ? 'chat-bubble-ai' : 'chat-bubble bg-primary/20'
                  }`}
                >
