...
export const insightsAPI = {
  ...
  weekly: async () => {
    return apiRequest('/insights/weekly');
  },

  trend: async (range: 'weekly' | '8weeks' = 'weekly') => {
    return apiRequest(`/insights/trend?range=${range}`);
  },

  history: async (limit: number = 10) => {
    return apiRequest(`/insights/history?limit=${limit}`);
  },
};
